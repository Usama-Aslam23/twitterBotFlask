...
df = pd.read_sql("SELECT * FROM twitter_users WHERE uid IN (SELECT MAX(uid) FROM twitter_users GROUP BY displayName)", db_connection)
dk = pd.read_sql(" SELECT * FROM keywords WHERE id IN (SELECT MAX(id) FROM keywords GROUP BY word)", db_connection)
def runTime(df, ds, dk):
